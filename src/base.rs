...
impl Base {
    ...
    pub(crate) fn ref_mut<T>(&mut self, offset: u32) -> &mut T {
        unsafe { &mut *((self as *mut Base as *mut u8).add(offset as usize) as *mut T) }
    }

    pub(crate) fn get_size(&self) -> usize {
        *self.ref_at::<u32>(0) as usize
    }

    pub(crate) fn has_bit(&self, bit: u8) -> bool {
        self.ref_at::<u32>((bit as usize / 32 + 1) * 4) & (1 << (bit % 32)) != 0
    }

    pub(crate) fn set_has_bit(&mut self, has_bit_idx: u32) {
        let has_bit_offset = has_bit_idx / 32;
        *self.ref_mut::<u32>((has_bit_offset + 1) * 4) |= 1 << (has_bit_idx as usize % 32);
    }
